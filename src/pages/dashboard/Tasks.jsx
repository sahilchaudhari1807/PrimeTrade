...
export default function Tasks() {
  ...
  // Helper to run search (shows small loading state for search button)
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
        ...
        {/* Right: add task form — full width on small screens, fixed width on md+ */}
        <div id="add-task" className="w-full md:w-1/3">
          <TaskForm
            onCreate={(task) => {
          ...
          {tasks.map((t) => (
            <li key={t._id} className="flex items-center justify-between gap-4 border p-3 rounded hover:shadow">
              <div className="min-w-0">
                ...
                {t.description && <div className="text-sm text-gray-600 mt-1 truncate">{t.description}</div>}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  aria-label={`Mark ${t.title} as ${t.completed ? "pending" : "completed"}`}
                >
